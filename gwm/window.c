...
#define TITLEBAR_FOCUS_COLOR    0x255ed5
...
enum window_click_action click_window(struct window *win, int x, int y) {
    if (x >= win->top_corner_x && x <= win->top_corner_x + win->length_x &&
        y >= win->top_corner_y)
    {
        if (y <= win->top_corner_y + FONT_HEIGHT) {
...
void draw_window(struct window *win, struct framebuffer *fb) {
    ...
    const int start_body_y = win->top_corner_y + FONT_HEIGHT;
    const int final_y      = win->top_corner_y + win->length_y;

    // Draw the window bar.
    const uint32_t title_color = win->is_focus ?
      TITLEBAR_FOCUS_COLOR : TITLEBAR_NO_FOCUS_COLOR;
    draw_rectangle(fb, start_x, start_y, final_x, start_body_y, title_color);
    ...
    draw_string(fb, start_x, start_y, win->name, char_count,
        TITLEBAR_FONT_COLOR, title_color);
